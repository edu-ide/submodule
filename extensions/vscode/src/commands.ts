...
import { AUTH_PROVIDER_ID, SCOPES, EduSenseProvider } from "./auth/EduSenseProvider";
// import { VsCodeMessenger } from "./webviewProtocol"; // 일단 주석 처리
import { registerPracticeCommands } from './commands/practiceCommands'; // registerPracticeCommands import 추가
...
const commandsMap: (
  ...
  quickEdit: QuickEdit,
  core: Core,
  eduSenseProvider: EduSenseProvider
  // vsCodeMessenger: VsCodeMessenger
  ...
  // vsCodeMessenger
    ...
    async function streamInlineEdit(
      ...
      const contextMenuPrompts = config.experimental?.contextMenuPrompts;
      let prompt = fallbackPrompt;
      if (contextMenuPrompts && promptName in contextMenuPrompts) {
        // Use 'any' to bypass strict index check - less type-safe but avoids the loop.
        prompt = (contextMenuPrompts as any)[promptName];
      }
    ...
    return {
      ...
      "pearai.getPearAuth": async () => {
        captureCommandTelemetry("getPearAuth");
        try {
          const session = await vscode.authentication.getSession(AUTH_PROVIDER_ID, SCOPES, { createIfNone: false });
          if (session) {
            console.log('Returning existing session info:', session.id);
            return {
              accessToken: session.accessToken,
              account: session.account,
            };
          } else {
            console.log('No active PearAI session found.');
            return undefined;
          }
        } catch (e) {
          console.error("Error getting PearAI auth info:", e);
          return undefined;
        }
      },
      "pearai.login": async () => {
        captureCommandTelemetry("login");
        try {
          console.log('[Login Command] Attempting getSession with forceNewSession: true...');
          const session = await vscode.authentication.getSession(
            AUTH_PROVIDER_ID,
            SCOPES,
            { forceNewSession: true }
          );
          console.log('[Login Command] getSession call completed.');

          if (session) {
            console.log('[Login Command] Session received:', session.id);
            console.log('Login successful, session ID:', session.id);
            vscode.window.showInformationMessage('PearAI 로그인 성공');
            // Line 885 관련: setIdeInfo 호출 주석 처리 (타입 확인 필요)
            // sidebar.webviewProtocol?.request("setIdeInfo", { ideType: "vscode", user: session.account.label }, [PEARAI_CHAT_VIEW_ID, PEARAI_SEARCH_VIEW_ID, PEARAI_OVERLAY_VIEW_ID]);
            // 대신 콘솔 로그 추가
            console.log('Login successful, user:', session.account.label);
          } else {
            console.log('[Login Command] No session object was returned.');
            console.log('Login cancelled or failed, no session created.');
          }
        } catch (e: any) {
          console.error("[Login Command] Error during getSession:", e);
          vscode.window.showErrorMessage(`PearAI 로그인 오류: ${e.message || e}`);
          console.error("Login error:", e);
        }
      },
      "pearai.logout": async () => {
        captureCommandTelemetry("logout");
        try {
          console.log('[Logout Command] Attempting logout...');
          // 1. 현재 세션 ID 가져오기
          const session = await vscode.authentication.getSession(AUTH_PROVIDER_ID, SCOPES, { createIfNone: false });

          if (session) {
            console.log(`[Logout Command] Found active session: ${session.id}. Requesting removal via provider.`);
            try {
              // --- 수정된 부분 ---
              // EduSenseProvider의 removeSession 메소드를 직접 호출
              await eduSenseProvider.removeSession(session.id);
              // removeSession 내부에서 SecretStorage 업데이트 및 _sessionChangeEmitter.fire()가 호출됨
              // ------------------
              console.log(`[Logout Command] Session removal requested successfully for ${session.id}.`);
              vscode.window.showInformationMessage('PearAI 로그아웃 되었습니다.');
              // UI 업데이트는 onDidChangeSessions 리스너에서 처리될 것임
            } catch (removeError: any) {
              console.error(`[Logout Command] Error calling eduSenseProvider.removeSession:`, removeError);
              vscode.window.showErrorMessage(`로그아웃 중 오류 발생: ${removeError.message || removeError}`);
            }
          } else {
            console.log('[Logout Command] No active session found to logout.');
            vscode.window.showInformationMessage('현재 로그인된 PearAI 세션이 없습니다.');
          }
        } catch (e: any) {
          console.error("[Logout Command] Error during getSession for logout:", e);
          vscode.window.showErrorMessage(`로그아웃 오류: ${e.message || e}`);
        }
      },
      "pearai.closeChat": () => {
        vscode.commands.executeCommand("workbench.action.toggleAuxiliaryBar");
...
export function registerAllCommands(
  ...
  const commandFunctions = commandsMap(
    ide,
    extensionContext,
    sidebar,
    configHandler,
  ...
  for (const command in commandFunctions) {
    // Remove registration for deprecated commands
    // if (command === "pearai.updateUserAuth" || command === "pearai.checkPearAITokens") {
    //   continue;
    // }
    extensionContext.subscriptions.push(
      vscode.commands.registerCommand(command, commandFunctions[command]),
    );
  }

  // === registerPracticeCommands 호출 수정 ===
  registerPracticeCommands(context, ide as any); // Pass ide instead of sidebar.webviewProtocol
  // ======================================
}
