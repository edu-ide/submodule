...
class VsCodeIde implements IDE {
  ...
  public readonly webviewProtocolPromise: Promise<VsCodeWebviewProtocol>;

  constructor(
    private readonly diffManager: DiffManager,
    vscodeWebviewProtocolPromise: Promise<VsCodeWebviewProtocol>,
  ) {
    this.ideUtils = new VsCodeIdeUtils();
    this.webviewProtocolPromise = vscodeWebviewProtocolPromise;
  }
  ...
  async createPracticeWorkspace(url: string, practiceId: string): Promise<void> {
    await vscode.commands.executeCommand('pearai.createPracticeWorkspace', { zipUrl: url, practiceId: practiceId });
  }
  ...
  async submitPractice(practiceId: string, requirements: string): Promise<void> {
    console.log(`VsCodeIde.submitPractice called for ${practiceId} with requirements (length: ${requirements?.length ?? 0}), executing command 'pearai.submitPractice'.`);
    await vscode.commands.executeCommand('pearai.submitPractice', { practiceId, requirements });
  }
}
